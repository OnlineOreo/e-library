...
export async function GET(request) {
  ...
  const formattedCatalogIds = catalogId
    .split(",")
    .map(id => `"${id.trim()}"`)
    .join(",");
  ...
  const solrUrl = `${process.env.NEXT_PUBLIC_SOLR_BASE_URL}/solr/${formattedCatalogCore}/select?indent=true&q.op=OR&q=${encodeURIComponent(solrQuery)}&rows=1`;
