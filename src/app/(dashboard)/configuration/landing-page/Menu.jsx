...
export default function Menu() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const activePublisherList = publisherList
      .filter((item) => item.is_active)
      .map((item) => item.id);
    const activeCategoriesList = categoriesList
      .filter((item) => item.is_active)
      .map((item) => item.id);
    const activeCollectionList = collectionList
      .filter((item) => item.is_active)
      .map((item) => item.id);
    const activeMediaIds = mediaList
      .filter((item) => item.is_active)
      .map((item) => item.id);
    ...
    <>
      <Form onSubmit={handleSubmit}>
        <Row className="p-4 user-select-none">
          ...
          />
          <Button className="w-100 mt-5" disabled={isLoading} type="submit">
