...
const ViewContentGroup = () => {
  ...
  const loadContentGroup = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/content-groups`, {
  ...
  const handleDelete = async (params) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            await axios.delete(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/content-groups?cg_id=${params.id}`, {
              headers: { Authorization: token },
            });
            Swal.fire("Deleted!", "Content Group has been deleted.", "success");
            setContentGroup((prev) => prev.filter(item => item.cg_id !== params.id));
          } catch (error) {
  ...
  const handleEdit = (params) => {
    router.push(`/library-department/content-group/edit/${params.id}`);
  };

  const filteredContentGroup = contentGroup.filter(inst =>
    inst.content_name.toLowerCase().includes(search.toLowerCase())
  );

  const columns = [
    // { field: "cg_id", headerName: "ID", width: 150 },
    { field: "content_name", headerName: "Content Name", flex: 2},
...
export default ViewContentGroup;
